...
class QuestionEmbedModel(nn.Module):
    def __init__(self, in_size, embed=32, hidden=128):
        ...
        self.wembedding = nn.Embedding(in_size, embed) # word embedding
        self.lstm = nn.LSTM(embed, hidden, batch_first=True) # lstm for question embedding
        self.hidden = hidden # hidden size of the lstm
...
class RN(BasicModel):
    def __init__(self, args, vocab_size):
        ...
        self.lstm = QuestionEmbedModel(vocab_size)
        
        self.relation_type = args.relation_type
        
        if self.relation_type == 'ternary':
            ##(number of filters per object+coordinate of object)*3+lstm output (128)
            self.g_fc1 = nn.Linear((24+2)*3+128, 256)
        else:
            ##(number of filters per object+coordinate of object)*2+lstm output (128)
            self.g_fc1 = nn.Linear((24+2)*2+128, 256)
        ...
        def cvt_coord(i):
            return [(i/8-2)/2., (i%8-2)/2.]
        
        self.coord_tensor = torch.FloatTensor(args.batch_size, 64, 2)
        if args.cuda:
            self.coord_tensor = self.coord_tensor.cuda()
        self.coord_tensor = Variable(self.coord_tensor)
        np_coord_tensor = np.zeros((args.batch_size, 64, 2))
        for i in range(64): # was previously 25
            np_coord_tensor[:,i,:] = np.array( cvt_coord(i) )
        self.coord_tensor.data.copy_(torch.from_numpy(np_coord_tensor))


        self.fcout = FCOutputModel(vocab_size)
        
        self.optimizer = optim.Adam(self.parameters(), lr=args.lr)


    def forward(self, img, qst):
        x = self.conv(img) ## x = (64 x 24 x 5 x 5) -> (6 x 24 x 8 x 8)
        # pass question through lstm
        qst = self.lstm(qst) ## qst = (64 x 128)
        
        """g"""
        mb = x.size()[0]
        n_channels = x.size()[1]
        d = x.size()[2]
        # x_flat = (64 x 25 x 24) -> (6 x 64 x 24)
        ...
        if self.relation_type == 'ternary':
            ...
            qst = qst.repeat(1, 64, 1)
            qst = torch.unsqueeze(qst, 2)

            # cast all pairs against each other
            x_i = torch.unsqueeze(x_flat, 1)  # (6x1x64x26+18)
            x_i = x_i.repeat(1, 64, 1, 1)  # (64x64x64x26+18)
            x_j = torch.unsqueeze(x_flat, 2)  # (64x25x1x26+18)
            x_j = torch.cat([x_j, qst], 3)
            x_j = x_j.repeat(1, 1, 64, 1)  # (64x25x25x26+18)
            
            # concatenate all together
            x_full = torch.cat([x_i,x_j],3) # (64x25x25x2*26+18) -> (6x64x64x180)
        
            # reshape for passing through network
            x_ = x_full.view(mb * (d * d) * (d * d), 180)  # (64*25*25x2*26*18) = (40.000, 70)
